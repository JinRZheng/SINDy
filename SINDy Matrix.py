# -*- coding: utf-8 -*-
"""
Created on Mon May 20 12:20:08 2024
...
# Add lines 26-27 to test the given specific matrix with minimizing Linear, NonLinear, or Total Error.
# Line 72 also change correspondly to Line 28 to be consistent.
def linear_3D(t,x):
    return [-0.1 * x[0] - 2 * x[1] +0* x[2], 2 * x[0] -0.1 * x[1] + 0 * x[2], 0*x[0] + 0 * x[1] -0.3 * x[2] ]
    

def SINDy_model(Threshold,initCondition=[2,0,1]):             # -0.1 -2    0
    #return the model                                         #  2   -0.1  0
                                                              #  0    0    -0.3    # SINDy matrix
...
def SINDyThresholdOptimization(model):
    ...
    modelerrorLinear = abs(a11-(-0.1)) + abs(a12-(-2)) + abs(a13-(0)) + abs(a21-(2)) + abs(a22-(-0.1)) + abs(a23-(0))+ abs(a31-(0)) + abs(a32-(0)) + abs(a33-(-0.3)) 
    # NonLinear Error is just absolute value of all the coefficent Minus the Linear Error
    
    #modelerrorNonLinear = sum(sum(abs(model.coefficients()))) - (a11 + a12 + a13 + a21 + a22 + a23 + a31 + a32 +a33)
    ...
    #errormatrix[0:3,1:4]=0 #set the linear part to zero 
...
for currentdigit in range(ndecimals):  # 
    errors = []
    ...
    for value in initSearchLineSpace:
        ...
        # Optimize/minimize linear Error
       # errors.append(currenterror[0])                     # for Minimize the Linear Error
        # errors.append(currenterror[1])                    # for Minimize the NonLinear Error
        errors.append(currenterror[0]+currenterror[1])    # for Minimize the Total Error
