# -*- coding: utf-8 -*-
"""
Created on Mon May 20 11:55:40 2024
...
def linear_3D(t,x):
    return [0.1 * x[0] - 2 * x[1] +1* x[2], -2 * x[0] +0.1 * x[1] + 1 * x[2], 1*x[0] + 1 * x[1] -0.3 * x[2] ]
    
                                                       #  0.1   -2      1
def SINDy_model(Threshold,initCondition=[2,0,1]):      #  -2    0.1     1
    #return the model                                  #   1     1     -0.3     SINDy does not even run for it!
...
def SINDyThresholdOptimization(model):
    ...
    modelerrorLinear = abs(a11-(0.1)) + abs(a12-(-2)) + abs(a13-(1)) + abs(a21-(-2)) + abs(a22-(0.1)) + abs(a23-(1))+ abs(a31-(1)) + abs(a32-(1)) + abs(a33-(-0.3)) 
...
ax1.plot(xaxis,errorsLinear,"-r", label="LinErr")
# Linear error in red color
#ax2.plot(xaxis,errorsNonLinear,color="#009900")
ax2.plot(xaxis,errorsNonLinear,"-g", label="NonLinErr")
