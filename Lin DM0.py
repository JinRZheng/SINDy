# -*- coding: utf-8 -*-
"""
Created on Mon May 20 15:24:35 2024
...
def linear_3D(t,x):
    return [-0.1 *x[0] + 2 *x[1] + 0*x[2], -2 *x[0] -0.1 *x[1] + 0 *x[2], 0 *x[0] + 2 *x[1] -0.3 *x[2] ]
    
                                                         # -0.1   2      0
def SINDy_model(Threshold,initCondition=[2,0,1]):        # -2     -0.1   0
    #return the model                                    # 0      2     -0.3 
...
def SINDyThresholdOptimization(model):
    ...
    modelerrorLinear = abs(a11-(-0.1)) + abs(a12-(2)) + abs(a13-(0)) + abs(a21-(-2)) + abs(a22-(-0.1)) + abs(a31-(0)) + abs(a32-(2)) + abs(a33-(-0.3)) 
